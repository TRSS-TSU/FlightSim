...
Codex may suggest scene changes but must not modify Unity GameObjects without approval.

If changes are required Codex must first present a plan including exact Unity hierarchy paths and inspector fields.

After approval Codex may apply changes using MCP and must summarize modifications.
